...
def deduplicate_answers(df: pd.DataFrame, threshold: float = 0.75) -> pd.DataFrame:
    ...
    signatures = (
        df['query_clean'].fillna('') + ' [SEP] ' + df['answer_clean'].fillna('')
    ).tolist()
...
def prepare():
    ...
    with open(JSONL_OUT, 'w') as f:
        for i, row in kb_df.iterrows():
            ...
            query = '' if pd.isna(row['query']) else str(row['query']).strip()
            answer = '' if pd.isna(row['answer']) else str(row['answer']).strip()
            text_parts = []
            if subject:
                text_parts.append(f"Subject: {subject}")
            if query:
                text_parts.append(f"Customer issue: {query}")
            if answer:
                text_parts.append(f"Resolution: {answer}")
            chunk = {
                "id"      : f"chunk_{i}",
                # embed subject + query + answer so templated replies remain distinguishable
                "text"    : "\n".join(text_parts),
                "answer"  : answer,
                "query"   : query,
                "subject" : subject,
